...
def _get_image_blob_batch(imgs):
...
def _get_blobs(im, rois):
    ...
    if not cfg.TEST.HAS_RPN: # True, not called
        blobs['rois'] = _get_rois_blob(rois, im_scale_factors)
    return blobs, im_scale_factors



def _get_blobs_batch(imgs, rois):
    """Convert an image and RoIs within that image into network inputs."""
    blobs = {'data' : None, 'rois' : None}
    blobs['data'], im_scale_factors = _get_image_blob_batch(imgs)
    if not cfg.TEST.HAS_RPN: # True, not called
        blobs['rois'] = _get_rois_blob(rois, im_scale_factors)
    return blobs, im_scale_factors
...
def im_detect_batch(net, imgs, boxes=None):
    ...
    """
    blobs, im_scales = _get_blobs_batch(imgs, boxes)

    # When mapping from image ROIs to feature map ROIs, there's some aliasing
    # (some distinct image ROIs get mapped to the same feature ROI).
    # Here, we identify duplicate feature ROIs, so we only compute features
    # on the unique subset.
    #print cfg.DEDUP_BOXES, cfg.TEST.HAS_RPN
    ...
    if cfg.TEST.SVM: # false
        ...
        scores = np.split(net.blobs['cls_score'].data, dividers)
    else:
        # use softmax estimated probabilities
        scores = np.split(blobs_out['cls_prob'], dividers)

    #print cfg.TEST.BBOX_REG True, cfg.TEST.SVM False
    if cfg.TEST.BBOX_REG:
        # Apply bounding-box regression deltas
        box_deltas = np.split(blobs_out['bbox_pred'], dividers)
        pred_boxes = []
        for idx in xrange(im_scales.shape[0]):
            pred_boxes += [bbox_transform_inv(boxes[idx], box_deltas[idx])]
